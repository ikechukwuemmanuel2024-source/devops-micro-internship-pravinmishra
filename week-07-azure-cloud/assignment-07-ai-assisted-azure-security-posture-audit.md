...
![Screenshot 0](screenshots/week7-ass7-00.png)
...
![Screenshot 0](screenshots/week7-ass7-01.png)
...
![Screenshot 0](screenshots/week7-ass7-02.png)
...
![Screenshot 0](screenshots/week7-ass7-05.png)
...
The AWS and Azure audits use different cloud services, but the finding categories are very similar. AWS Security Groups map to Azure Network Security Groups for checking unrestricted SSH or RDP access. S3 public access maps to Azure Storage Account public blob access. AWS EBS encryption maps to Azure managed disk encryption, while Amazon RDS public access maps to Azure Database for MySQL public network access.

What stayed exactly the same was the security-audit workflow: first gather evidence using read-only commands, then analyze the findings, explain the security impact, recommend a remediation, have the human operator review and execute the fix, and finally run the audit again to verify that the finding was resolved. The commands changed from `aws` to `az`, but the Agentic Loop — Gather → Analyze → Human Act → Verify — remained the same.
